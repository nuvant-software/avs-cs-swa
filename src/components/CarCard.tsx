...
const CarCard: React.FC<Props> = ({ car, layout = "grid" }) => {
  ...
  // 📸 Alle plaatjes van deze auto inladen
  const allImages = useMemo(() => {
    const imports = import.meta.glob(
      "/src/data/cars/**/images/*.{jpg,jpeg,png}",
      { eager: true, as: "url" }
    );
    return Object.entries(imports)
      .filter(([path]) => path.includes(`car_${car.id}/images/`))
  ...
  // Helper om een plaatje te renderen
  ...
  // Bepaal wat in de hover‑zones komt
  const getZoneContent = () => {
    ...
    // ────────── HIER AANGEPAST ──────────
    // Toon index 1 enkel als hoverZone===1, anders altijd index 0
